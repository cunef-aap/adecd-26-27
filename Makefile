...
# Uso habitual:  make sitio && make notebooks

.PHONY: check marcas sitio notebooks preview revision publicar datos limpiar pdfs publicado
...
notebooks: check             ## cuadernos completos -> docs/live-notebooks/
	python scripts/crear-ipynb.py
	mkdir -p docs/datos
	cp datos/prostate.data datos/airbnb_madrid.csv docs/datos/

revision:                    ## configura el material adicional de revision.txt
	python scripts/publicado.py --revision

preview: revision            ## servidor privado con recarga, no modifica docs/
	quarto preview --profile revision
...
pdfs: check                  ## PDF de todo el libro, incluidos solucionarios -> pdf/
	...
	quarto render --profile completo
